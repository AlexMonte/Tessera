...
impl PortRole {
    ...
    pub fn is_gate(&self) -> bool {
        matches!(self, PortRole::Gate)
    }

    pub fn is_signal(&self) -> bool {
        matches!(self, PortRole::Signal)
    }

    pub fn is_sequence(&self) -> bool {
        matches!(self, PortRole::Sequence)
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum PieceTemporalKind {
    #[default]
    Value,
    Trigger,
    Sequence,
...
impl PieceTemporalKind {
    pub fn is_value(&self) -> bool {
        matches!(self, PieceTemporalKind::Value)
    }
}
...
impl FanInPolicy {
    pub fn is_single(&self) -> bool {
        matches!(self, FanInPolicy::Single)
    }
}
...
impl FanOutPolicy {
    pub fn is_broadcast(&self) -> bool {
        matches!(self, FanOutPolicy::Broadcast)
    }
}
...
mod tests {
    use super::{
        DomainBridgeKind, ExecutionDomain, FanInPolicy, FanOutPolicy, PieceCategory,
        PieceSemanticKind, PieceTemporalKind, PortRole, PortType, PortTypeConnectionError,
        Rational, TemporalEdgeKind, TemporalNodeKind, TileSide,
    };
    ...
    fn piece_category_and_semantic_kind_serialize_snake_case() {
        ...
        assert_eq!(
            serde_json::to_string(&PieceCategory::Cadence).unwrap(),
            "\"cadence\""
        );
        assert_eq!(
        ...
        assert_eq!(
            serde_json::to_string(&PieceSemanticKind::Sequence).unwrap(),
            "\"sequence\""
        );
    }

    #[test]
    fn role_and_temporal_helpers_expose_rhythm_semantics() {
        assert!(PortRole::Gate.is_gate());
        assert!(PortRole::Signal.is_signal());
        assert!(PortRole::Sequence.is_sequence());
        assert!(PieceTemporalKind::Value.is_value());
        assert!(FanInPolicy::Single.is_single());
        assert!(FanOutPolicy::Broadcast.is_broadcast());
        assert_eq!(
            serde_json::to_string(&TemporalNodeKind::Boundary).unwrap(),
            "\"boundary\""
        );
        assert_eq!(
            serde_json::to_string(&TemporalEdgeKind::DelayFeedback).unwrap(),
            "\"delay_feedback\""
        );
    }
