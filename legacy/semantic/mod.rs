...
use crate::internal::{StructuralEdgeInfo, curate_diagnostics, roles_compatible};
use crate::pattern::analyze_pattern_surface;
use crate::piece::PieceDef;
use crate::piece_registry::PieceRegistry;
use crate::types::{
    DomainBridge, EdgeId, GridPos, PortRole, PortType, TemporalEdgeKind, TemporalNodeKind,
};
...
fn finalize_full_analysis(
    ...
    let temporal_nodes = classify_temporal_nodes(graph, registry);
    let temporal_edges =
        classify_temporal_edges(graph, &nodes, &scaffold.topo.delay_edges, &temporal_nodes);
    diagnostics.extend(collect_role_compatibility_diagnostics(&nodes, None));
    diagnostics.extend(collect_pattern_source_diagnostics(&nodes, None));
...
fn analyze_incremental(
    ...
    diagnostics.extend(collect_pattern_source_diagnostics(&nodes, Some(&affected)));
...
fn is_incremental_node_diagnostic(diagnostic: &Diagnostic) -> bool {
    matches!(diagnostic.kind, DiagnosticKind::DelayTypeMismatch { .. })
        || matches!(diagnostic.kind, DiagnosticKind::RoleMismatch { .. })
        || matches!(
            diagnostic.kind,
            DiagnosticKind::PatternOperatorMissingTarget { .. }
                | DiagnosticKind::PatternOperatorMissingArgument { .. }
                | DiagnosticKind::PatternConsecutiveOperators { .. }
                | DiagnosticKind::PatternTypeMismatch { .. }
                | DiagnosticKind::InvalidPatternPlacement { .. }
        )
...
fn collect_pattern_source_diagnostics(
    nodes: &BTreeMap<GridPos, AnalyzedNode>,
    affected: Option<&BTreeSet<GridPos>>,
) -> Vec<Diagnostic> {
    let mut diagnostics = Vec::new();
    ...
    for (pos, node) in nodes {
        ...
        let Some(pattern_source) = &node.pattern_source else {
            continue;
        };

        for diagnostic in &pattern_source.diagnostics {
            let kind = match diagnostic.code {
                crate::pattern::PatternDiagnosticCode::OperatorMissingTarget => {
                    DiagnosticKind::PatternOperatorMissingTarget {
                        path: diagnostic.path.clone(),
                    }
                }
                crate::pattern::PatternDiagnosticCode::OperatorMissingArgument => {
                    DiagnosticKind::PatternOperatorMissingArgument {
                        path: diagnostic.path.clone(),
                    }
                }
                crate::pattern::PatternDiagnosticCode::ConsecutiveOperators => {
                    DiagnosticKind::PatternConsecutiveOperators {
                        path: diagnostic.path.clone(),
                    }
                }
                crate::pattern::PatternDiagnosticCode::TypeMismatch => {
                    DiagnosticKind::PatternTypeMismatch {
                        path: diagnostic.path.clone(),
                        reason: diagnostic.message.clone(),
                    }
                }
                crate::pattern::PatternDiagnosticCode::InvalidPlacement => {
                    DiagnosticKind::InvalidPatternPlacement {
                        path: diagnostic.path.clone(),
                        reason: diagnostic.message.clone(),
                    }
            ...
            diagnostics.push(Diagnostic::error(kind, Some(*pos)));
        }
    }
        ...
        .map(|(pos, node)| {
            let analyzed = registry
                ...
                .unwrap_or_else(|| AnalyzedNode {
                    ...
                    pattern_source: node.pattern_source.as_ref().map(analyze_pattern_surface),
...
fn classify_temporal_nodes(
    graph: &Graph,
    registry: &PieceRegistry,
) -> BTreeMap<GridPos, TemporalNodeKind> {
    graph
        .nodes
        .iter()
        .filter_map(|(pos, node)| {
            let piece = registry.get(node.piece_id.as_str())?;
            let kind = if piece.def().is_output() {
                TemporalNodeKind::Boundary
            } else {
                match piece.def().temporal_kind() {
                    crate::types::PieceTemporalKind::Value => TemporalNodeKind::Value,
                    crate::types::PieceTemporalKind::Trigger => TemporalNodeKind::Trigger,
                    crate::types::PieceTemporalKind::Sequence => TemporalNodeKind::Sequence,
                    crate::types::PieceTemporalKind::State => TemporalNodeKind::State,
                    crate::types::PieceTemporalKind::Delay => TemporalNodeKind::Delay,
                }
            };
            Some((*pos, kind))
        })
...
fn classify_temporal_edges(
    graph: &Graph,
    nodes: &BTreeMap<GridPos, AnalyzedNode>,
    delay_edges: &BTreeSet<EdgeId>,
    temporal_nodes: &BTreeMap<GridPos, TemporalNodeKind>,
) -> BTreeMap<EdgeId, TemporalEdgeKind> {
    graph
        .edges
        .values()
        .map(|edge| {
            let kind = if delay_edges.contains(&edge.id) {
                TemporalEdgeKind::DelayFeedback
            } else {
                let source_role = nodes
                    .get(&edge.from)
                    .map(|node| node.output_role.clone())
                    .unwrap_or_default();
                let target_role = nodes
                    .get(&edge.to_node)
                    .and_then(|node| node.input_roles.get(&edge.to_param))
                    .cloned()
                    .unwrap_or_default();
                let source_kind = temporal_nodes
                    .get(&edge.from)
                    .copied()
                    .unwrap_or(TemporalNodeKind::Value);
                let target_kind = temporal_nodes
                    .get(&edge.to_node)
                    .copied()
                    .unwrap_or(TemporalNodeKind::Value);

                if source_role.is_sequence() || target_role.is_sequence() {
                    TemporalEdgeKind::Sequence
                } else if source_role.is_gate() || target_role.is_gate() {
                    TemporalEdgeKind::Trigger
                } else if matches!(
                    source_kind,
                    TemporalNodeKind::State | TemporalNodeKind::Delay
                ) || matches!(
                    target_kind,
                    TemporalNodeKind::State | TemporalNodeKind::Delay
                ) {
                    TemporalEdgeKind::State
                } else {
                    TemporalEdgeKind::Value
                }
            };
