...
use crate::types::{
    EdgeId, ExecutionDomain, GridPos, PieceCategory, PieceSemanticKind, PortRole, PortType,
    TileSide,
};
...
fn analyze_subgraph_preserves_input_metadata() {
        ...
        nodes: BTreeMap::from([
            (
                ...
                Node {
                    ...
                    inline_params: BTreeMap::from([
                        ...
                        ("role".into(), Value::String("signal".into())),
            ...
            (
                ...
                Node {
                    piece_id: SUBGRAPH_OUTPUT_ID.into(),
                    inline_params: BTreeMap::from([
                        ("role".into(), Value::String("sequence".into())),
                        ("category".into(), Value::String("cadence".into())),
                        ("description".into(), Value::String("Cadence helper".into())),
                        ("tags".into(), Value::String("subgraph, cadence".into())),
                    ]),
                    pattern_source: None,
                    input_sides: BTreeMap::from([("input".into(), TileSide::LEFT)]),
                    output_side: None,
    ...
    assert_eq!(signature.output_role, PortRole::Sequence);
    assert_eq!(signature.category, Some(PieceCategory::Cadence));
    assert_eq!(signature.description.as_deref(), Some("Cadence helper"));
    assert_eq!(signature.tags, vec!["subgraph", "cadence"]);
}
...
fn subgraph_pieces_build_generated_piece_defs() {
    let signature = SubgraphSignature {
        ...
        output_role: PortRole::Value,
        category: Some(PieceCategory::Cadence),
        description: Some("Echo a phrase".into()),
        tags: vec!["subgraph".into(), "phrase".into()],
    };
    ...
    assert_eq!(pieces[0].def().category, PieceCategory::Cadence);
    assert_eq!(pieces[0].def().tags, vec!["subgraph", "phrase"]);
}
