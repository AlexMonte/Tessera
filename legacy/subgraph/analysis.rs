...
use super::helpers::{
    slot_from_piece_id, split_inline_tags, subgraph_boundary_port_type, subgraph_boundary_role,
    subgraph_piece_category, trimmed_inline_text,
};
use super::types::{
...
pub fn analyze_subgraph(
    ...
    let mut output_role = crate::types::PortRole::Value;
    ...
    for (pos, node) in &graph.nodes {
        if is_subgraph_input_id(node.piece_id.as_str()) {
            ...
            inputs.push(SubgraphInput {
                ...
                label,
                port_type,
                role: subgraph_boundary_role(&node.inline_params),
                required,
                is_receiver,
                default_value,
            });
            ...
            output_role = subgraph_boundary_role(&node.inline_params);
            category = subgraph_piece_category(&node.inline_params);
            description = trimmed_inline_text(&node.inline_params, "description");
            tags = split_inline_tags(&node.inline_params);
        }
    }
