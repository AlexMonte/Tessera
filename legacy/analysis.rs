...
use crate::pattern::AnalyzedPatternSource;
use crate::types::{
    DomainBridge, DomainBridgeKind, EdgeId, GridPos, PortRole, PortType, TemporalEdgeKind,
    TemporalNodeKind, TileSide,
};
...
pub struct AnalyzedGraph {
    ...
    /// Temporal classification per analyzed node for rhythm-first runtimes.
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty")]
    pub temporal_nodes: BTreeMap<GridPos, TemporalNodeKind>,
    /// Temporal classification per accepted edge.
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty")]
    pub temporal_edges: BTreeMap<EdgeId, TemporalEdgeKind>,
}
...
pub struct AnalyzedNode {
    ...
    /// Resolved inner pattern source facts for nodes that own a pattern surface.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub pattern_source: Option<AnalyzedPatternSource>,
    ...
    pub(crate) fn invalidate_from_apply_outcome(
        ...
        for op in applied_ops {
            match op {
                ...
                | GraphOp::NodeSetPatternSurface { position, .. }
