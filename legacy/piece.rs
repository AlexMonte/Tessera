...
use crate::types::{
    DELAY_PIECE_ID, FanInPolicy, FanOutPolicy, GridPos, PieceCategory, PieceSemanticKind,
    PieceTemporalKind, PortRole, PortType, Rational, TileSide,
};
...
impl ParamSchema {
    ...
    pub fn can_inline(&self) -> bool {
        match self {
            ...
            ParamSchema::Custom { can_inline, .. } | ParamSchema::Pattern { can_inline, .. } => {
                *can_inline
            }
        }
    }
    ...
    pub fn default_value(&self) -> Option<Value> {
        match self {
            ...
            ParamSchema::Pattern { default, .. } => default::clone(),
...
pub struct PieceDef {
    ...
    #[serde(default, skip_serializing_if = "PieceTemporalKind::is_value")]
    pub temporal_kind: PieceTemporalKind,
    #[serde(default, skip_serializing_if = "FanInPolicy::is_single")]
    pub fan_in: FanInPolicy,
    #[serde(default, skip_serializing_if = "FanOutPolicy::is_broadcast")]
...
impl PieceDef {
    ...
    pub fn execution_kind(&self) -> PieceExecutionKind {
        if self.is_connector() {
            ...
            || matches!(
                self.temporal_kind,
                PieceTemporalKind::Delay | PieceTemporalKind::State
            )
        {
    ...
    pub fn temporal_kind(&self) -> PieceTemporalKind {
        if self.id == DELAY_PIECE_ID {
            PieceTemporalKind::Delay
        } else if !self.temporal_kind.is_value() {
            self.temporal_kind
        } else if self.output_role.is_sequence() {
            PieceTemporalKind::Sequence
        } else if self.output_role.is_gate() {
            PieceTemporalKind::Trigger
        } else if matches!(self.execution_kind(), PieceExecutionKind::State) {
            PieceTemporalKind::State
        } else {
...
mod tests {
    ...
    use crate::types::{
        DELAY_PIECE_ID, FanInPolicy, FanOutPolicy, PieceCategory, PieceSemanticKind,
        PieceTemporalKind, PortRole, PortType, Rational, TileSide,
    };
    use serde_json::json;
    ...
    fn piece_execution_kind_tracks_kernel_taxonomy() {
        ...
        let state = PieceDef {
            ...
            output_side: Some(TileSide::RIGHT),
            output_role: Default::default(),
            temporal_kind: PieceTemporalKind::Delay,
            fan_in: Default::default(),
            fan_out: Default::default(),
            description: None,
            tags: vec![],
    ...
    #[test]
    fn piece_def_exposes_temporal_and_fan_metadata() {
        let piece = PieceDef {
            id: "test.sequence".into(),
            label: "sequence".into(),
            category: PieceCategory::Cadence,
            semantic_kind: PieceSemanticKind::Sequence,
            namespace: "core".into(),
            params: vec![],
            output_type: Some(PortType::any()),
            output_side: Some(TileSide::RIGHT),
            output_role: PortRole::Sequence,
            temporal_kind: PieceTemporalKind::Sequence,
            fan_in: FanInPolicy::Single,
            fan_out: FanOutPolicy::Broadcast,
            description: None,
            tags: vec!["cadence".into()],
        };

        assert_eq!(piece.temporal_kind(), PieceTemporalKind::Sequence);
        assert!(piece.fan_in.is_single());
        assert!(piece.fan_out.is_broadcast());
    }
}
